...
if __name__ == "__main__":
    ...
    # IP addresses for the outgoing interfaces
    ips = [sys.argv[1], sys.argv[2], sys.argv[3]]
    ...
    interfaces = ["enp7s0", "enp8s0", "enp9s0"]

    # Pair each IP with its corresponding interface
    ip_interface_pairs = zip(ips, interfaces)

    # Create an iterator that cycles through the IP-interface pairs
    cycle_ip_interface_pairs = itertools.cycle(ip_interface_pairs)

    # Send each message to a different IP-interface pair
    for msg in messages:
        ip, iface = next(cycle_ip_interface_pairs)
        sendPacket(ip, msg, iface=iface)
